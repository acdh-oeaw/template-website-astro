 ...
 */
export function trackPageView(url: URL, locale: IntlLocale): void {
...
export function initAnalytics(locale: IntlLocale): void {
	function onPageLoad() {
		const url = new URL(window.location.href);
		trackPageView(url, locale);
	}

	/** Track page views with `ClientRouter` from `astro:transitions`. */
